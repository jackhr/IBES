...
final class EndpointGuard
{
    ...
    {
        $scopeKey = strtoupper((string) preg_replace('/[^a-z0-9]+/i', '_', $scope));

        $defaultMax = self::positiveInt((string) \Config::get('RATE_LIMIT_MAX', '15'), 15);
        $defaultWindow = self::positiveInt((string) \Config::get('RATE_LIMIT_WINDOW', '900'), 900);

        $max = self::positiveInt((string) \Config::get($scopeKey . '_RATE_LIMIT_MAX', (string) $defaultMax), $defaultMax);
        $window = self::positiveInt((string) \Config::get($scopeKey . '_RATE_LIMIT_WINDOW', (string) $defaultWindow), $defaultWindow);
