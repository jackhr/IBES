...
final class CaptchaVerifier
{
    ...
    {
        $provider = strtolower(trim((string) config('client.captcha.provider', 'none')));
        $enabled = (bool) config('client.captcha.enabled', $provider !== 'none');
    ...
    {
        return match ($provider) {
            'hcaptcha' => config('client.captcha.hcaptcha.secret_key'),
            'recaptcha' => config('client.captcha.recaptcha.secret_key'),
            default => null,
        };
    }
    ...
    {
        return match ($provider) {
            'hcaptcha' => (string) config('client.captcha.hcaptcha.verify_url', 'https://api.hcaptcha.com/siteverify'),
            'recaptcha' => (string) config('client.captcha.recaptcha.verify_url', 'https://www.google.com/recaptcha/api/siteverify'),
            default => '',
        };
    }
    ...
    {
        $explicit = config('client.captcha.hcaptcha.site_key');
    ...
    {
        $transport = strtolower(trim((string) config('client.captcha.http_transport', 'auto')));
